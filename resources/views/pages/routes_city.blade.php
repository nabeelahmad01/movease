...
<style>
  ...
  .list-tile { display:block; background:#fff; border:1px solid #eee; border-radius:10px; padding:14px 16px; color:#333; text-decoration:none; transition:all .2s; }
  .list-tile:hover { transform: translateY(-2px); box-shadow:0 10px 20px rgba(0,0,0,.06); }
          ...
          <form method="GET" action="{{ route('front.get.quote') }}">
            <div class="row g-2">
              <div class="col-md-4">
                ...
                <input type="text" id="rc_zip_from" name="zip_from" class="form-control zipfrom" placeholder="ZIP or City" required autocomplete="off">
              </div>
              <div class="col-md-4">
                <label class="form-label text-muted">Moving To*</label>
                <input type="text" id="rc_zip_to" name="zip_to" class="form-control zipto " placeholder="ZIP or City" required autocomplete="off">
              </div>
              <div class="col-md-4">
                <label class="form-label text-muted">Moving Date</label>
                <input type="text" id="rc_move_date" name="move_date" class="form-control movedate" placeholder="YYYY-MM-DD">
              </div>
              <div class="col-12">
...
(function(){
  ...
  if(window.flatpickr){ flatpickr('#rc_move_date', { dateFormat: 'Y-m-d', allowInput: true }); }
})();
</script>
@endpush
      ...
      <div class="col-lg-7">
        ...
        <div class="row g-3">
          @forelse($popularCityRoutes as $r)
            ...
            @endphp
            <div class="col-12">
              <a class="list-tile" href="{{ route('front.routes.city.slug', [$stateSlug, $fromSlug, $toSlug]) }}">
                <div class="d-flex justify-content-between">
                  <div>
                    <strong>{{ $r->fromCity->name ?? '' }}</strong> to <strong>{{ $r->toCity->name ?? '' }}</strong>
                    <div class="text-muted small">~ {{ $r->miles ?? '—' }} miles</div>
                  </div>
                  <div class="text-end">
                    <span class="badge bg-success">From {{ $r->min_cost ? '$'.number_format($r->min_cost) : '—' }}</span>
                    <span class="badge bg-warning text-dark">Up to {{ $r->max_cost ? '$'.number_format($r->max_cost) : '—' }}</span>
                  </div>
                </div>
              </a>
            </div>
          @empty
            <div class="col-12"><div class="alert alert-light mb-0">No routes found.</div></div>
          @endforelse
        </div>
      </div>
      <div class="col-lg-5">
        ...
        <div class="row g-2">
          @foreach($cities as $c)
            <div class="col-6">
              <a class="list-tile" href="{{ route('front.city.movers', [Str::slug($state->name), Str::slug($c->name)]) }}">
                {{ $c->name }}
              </a>
            </div>
          @endforeach
