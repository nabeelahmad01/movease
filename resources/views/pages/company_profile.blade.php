...
@section('meta_description',
    'Read reviews and get information about ' .
    $company->name .
    '. Compare moving services,
    prices, and customer feedback for your interstate move.')
@section('meta_keywords', $company->name . ', moving company reviews, interstate movers, moving services, ' .
    ($company->state->name ?? 'nationwide') . ' movers')
                    ...
                    <div class="row align-items-center">
                        ...
                        <div class="col-md-6">
                            ...
                            <p class="text-muted mb-3">
                                {{ $company->description ?: 'Professional moving services with years of experience. FMCSA licensed and fully insured.' }}
                            </p>
                ...
                <div class="reviews-list">
                            ...
                            <div class="review-meta">
                                <div>
                                    ...
                                    @if ($review->pickup_city && $review->delivery_city)
                                        <span class="me-3"><strong>Route:</strong> {{ $review->pickup_city }} to
                                            {{ $review->delivery_city }}</span>
                                    @endif
                                    @if ($review->move_date)
                                        <span><strong>Date:</strong>
                                            {{ \Carbon\Carbon::parse($review->move_date)->format('M Y') }}</span>
                                    @endif
                                </div>
                    ...
                    @empty
                        <div class="text-center py-5">
                            ...
                            <a href="{{ route('front.review.form', $company->slug) }}" class="btn btn-primary">Write a
                                Review</a>
                        </div>
                    @endforelse
                </div>
                ...
                <div class="faq-section">
                    ...
                    <div class="faq-item">
                        ...
                        <div class="faq-answer" id="faq1">
                            {{ $company->name }} offers comprehensive moving services including long-distance interstate
                            moves, local moves, packing services, storage solutions, and specialty item handling for pianos,
                    ...
                    <div class="faq-item">
                        ...
                        <div class="faq-answer" id="faq2">
                            Moving costs are calculated based on distance, weight of belongings, additional services
                            requested, and seasonal factors. They provide free in-home estimates for accurate pricing.
                        </div>
                    </div>
                    <div class="faq-item">
                        ...
                        <div class="faq-answer" id="faq3">
                            @if ($company->license_number)
                                Yes, {{ $company->name }} is fully licensed by the FMCSA for interstate moves and carries
                                comprehensive liability and cargo insurance to protect your belongings during transit.
                            @else
                                {{ $company->name }} carries comprehensive liability and cargo insurance to protect your
                                belongings during transit. Please contact them directly for licensing information.
                            @endif
                        </div>
                    </div>
                    <div class="faq-item">
                        ...
                        <div class="faq-answer" id="faq4">
                            Delivery timeframes vary by distance but typically range from 1-14 business days for
                            long-distance moves. They provide estimated delivery windows during booking.
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="sidebar-sticky">
                    <div class="sidebar">
                        ...
                        <form class="quote-form mb-4" action="/get-quote" method="GET">
                            ...
                            <div class="mb-3">
                                <input type="text" class="form-control zipfrom" id="cp_zip_from" name="zip_from"
                                    placeholder="Moving From" required autocomplete="off">
                            </div>
                            <div class="mb-3">
                                <input type="text" class="form-control zipto" id="cp_zip_to" name="zip_to"
                                    placeholder="Moving To" required autocomplete="off">
                            </div>
                            <div class="mb-3">
                                <input type="text" class="form-control movedate" id="cp_move_date" name="move_date"
                                ...
                                <li onclick="window.location.href='/company/{{ $relatedCompany->slug }}'">
                                    ...
                                    <div>
                                        ...
                                        <div class="small text-muted">
                                            {{ number_format($relatedCompany->averageRating(), 1) }}★
                                            ({{ $relatedCompany->reviews()->count() }} reviews)
                                        </div>
                                    </div>
                                </li>
                            @endforeach
                    ...
                    <div class="sidebar">
                        ...
                        <ul class="info-links">
                            @if ($company->state)
                                <li><a
                                        href="/movers/{{ strtolower(str_replace(' ', '-', $company->state->name ?? $company->state)) }}">{{ $company->state->name ?? $company->state }}
                                        Movers</a></li>
                            @endif
        ...
        <div class="row mt-5">
            <div class="col-lg-6">
                <div class="footer-info">
                    ...
                    <p>{{ $company->description ?: $company->name . ' has been serving customers across the United States for many years. We specialize in long-distance interstate moves and pride ourselves on providing reliable, professional moving services at competitive prices.' }}
                    </p>
                    <p>Our team of experienced movers is trained to handle all types of relocations, from small apartments
                        to large family homes. We use modern equipment and techniques to ensure your belongings arrive
                ...
                <div class="footer-info">
                    ...
                    @if ($company->address_line1)
                        ...
                        <p><i class="fas fa-map-marker-alt text-primary me-2"></i>
                            ...
                            @endif
                            @if ($company->state)
                                , {{ $company->state->name ?? $company->state }}
                            @endif
                            @if ($company->zip)
                                , {{ $company->zip }}
                            @endif
...
@push('scripts')
    <script>
        ...
        document.querySelectorAll('.filter-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                reviews.forEach(review => {
                    ...
                    if (filter === 'all' ||
                        (filter === rating) ||
                        (filter === 'recent' && review.querySelector('.text-muted').textContent
                            .includes('2024')) ||
        ...
        document.querySelectorAll('.vote-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                if (icon.classList.contains('fa-thumbs-up')) {
                    ...
                    const currentCount = parseInt(text.match(/\d+/)[0]);
                    this.innerHTML =
                        `<i class="fas fa-thumbs-down me-1"></i>Not Helpful (${currentCount + 1})`;
        ...
        document.querySelectorAll(
            '.review-item, .sidebar, .company-profile, .company-comparison, .faq-section, .footer-info').forEach(
            item => {
